// ========================================
// BOT CRAFT - PROFESSIONAL SCRIPT v3.2
// جلسات دائمة، كاميرا على منافذ 8080+
// ========================================
...
function loadAnalytics() {
    fetch('/api/bots')
        ...
        .then(data => {
            ...
            Promise.all(bots.map(bot => fetch(`/api/bot-stats/${bot.id}`).then(r => r.json()).catch(() => ({})))).then(statsArray => {
...
console.log('%c🚀 BotCraft v3.2 - جلسات دائمة، كاميرا على منافذ 8080+', 'color: #7c3aed; font-size: 16px; font-weight: bold;');