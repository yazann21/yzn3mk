...
function startBot(id) {
    fetch('/api/start-cloud-bot', {
        ...
        if (data.error === 'need_minecraft_auth') alert('⚠️ يجب التحقق من البوت أولاً (اضغط زر تحقق)');
...
function saveEditBot() { 
    fetch('/api/update-bot', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({
...
function refreshLogs() { if (currentLogsBotId) fetch(`/api/bot-logs/${currentLogsBotId}`, { credentials: 'include' }).then(res => res.json()).then(data => { if (document.getElementById('logsText')) document.getElementById('logsText').innerHTML = (data.logs || []).join('\n'); }); }
...
function fetchBotStats(id) {
    fetch(`/api/bot-stats/${id}`, { credentials: 'include' }).then(res => res.json()).then(data => {
        const statHealth = document.getElementById('statHealth'); if (statHealth) statHealth.innerHTML = data.health || '20';
        const statFood = document.getElementById('statFood'); if (statFood) statFood.innerHTML = data.food || '20';
        const statPosition = document.getElementById('statPosition'); if (statPosition) statPosition.innerHTML = data.position || '0,0,0';
        const statArmor = document.getElementById('statArmor'); if (statArmor) statArmor.innerHTML = data.armor || 'لا يوجد';
        const statWeapon = document.getElementById('statWeapon'); if (statWeapon) statWeapon.innerHTML = data.weapon || 'لا يوجد';
        const statLevel = document.getElementById('statLevel'); if (statLevel) statLevel.innerHTML = data.level || '0';
        const detailLevel = document.getElementById('detailLevel'); if (detailLevel) detailLevel.innerHTML = data.level || '0';
        const detailXp = document.getElementById('detailXp'); if (detailXp) detailXp.innerHTML = data.xp || '0';
        const detailKills = document.getElementById('detailKills'); if (detailKills) detailKills.innerHTML = data.kills || '0';
        const detailDeaths = document.getElementById('detailDeaths'); if (detailDeaths) detailDeaths.innerHTML = data.deaths || '0';
    }).catch(() => {});
}

function fetchInventory(id) {
    fetch(`/api/bot-inventory/${id}`, { credentials: 'include' }).then(res => res.json()).then(data => {
        if (data.inventory) {
...
function sendCommand(cmd, extra) { if (!controlBotId) return; fetch('/api/bot-command', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ botId: controlBotId, command: cmd, extra }) }); }
