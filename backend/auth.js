...
// استخدام الذاكرة المؤقتة للجلسات (أسهل ولا يسبب أخطاء)
app.use(session({
...
app.get('/auth/login', async (req, res) => {
  try {
    ...
    console.log('Auth URL:', url); // للتأكد
    res.json({ url });
  } catch (error) {
...
app.post('/api/start-cloud-bot', (req, res) => {
  ...
  if (!req.session.isRealMinecraft) {
    return res.status(400).json({ error: 'need_minecraft_auth', message: 'حساب مايكروسوفت غير مرتبط بماينكرافت حقيقي' });
  }
