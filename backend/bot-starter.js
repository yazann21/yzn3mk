...
function startBot(botId, username, uuid, serverIp, botType, teamNames = '', version = '1.21.10') {
  const botProcess = spawn('node', [path.join(__dirname, 'bot.js'), username, uuid, serverIp, botType, botId, teamNames, version], {
    env: { ...process.env, MC_USERNAME: username, MC_UUID: uuid, SERVER_IP: serverIp, BOT_TYPE: botType, BOT_ID: botId, TEAM_NAMES: teamNames, MC_VERSION: version },
...
function stopBot(botId) { const p = botProcesses.get(botId); if (p) { p.kill(); botProcesses.delete(botId); return true; } return false; }
function getBotLogs(botId) { return botLogs.get(botId) || []; }
function getBotStats(botId) { return botStats.get(botId) || { health: '--', food: '--', position: '--', armor: '--', weapon: '--' }; }
function getBotInventory(botId) { return botInventory.get(botId) || { inventory: [] }; }
