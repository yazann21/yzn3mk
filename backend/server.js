...
app.post('/api/clear-logs/:botId', (req, res) => { const p = path.join(__dirname, 'logs', `bot-${req.params.botId}.log`); if (require('fs').existsSync(p)) require('fs').writeFileSync(p, ''); res.json({ success: true }); });
app.get('/api/tasks/:botId', (req, res) => { db.all('SELECT * FROM tasks WHERE bot_id = ?', [req.params.botId], (err, tasks) => { res.json({ tasks: tasks || [] }); }); });
app.get('/camera/:botId', (req, res) => { res.redirect(`http://localhost:${8080 + parseInt(req.params.botId)}`); });
