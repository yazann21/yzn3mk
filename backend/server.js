...
app.get('/auth/bot-callback', async (req, res) => {
    ...
    try {
        ...
        res.send('<html><body style="background:#0a0a1a;color:white;text-align:center;padding:50px;"><h2>✅ تم التحقق من البوت!</h2><p>يمكنك إغلاق هذه النافذة.</p><script>setTimeout(()=>window.close(),3000)</script></body></html>');
    } catch (error) {
        console.error(error);
        res.status(500).send('فشل التحقق');
    }
});
