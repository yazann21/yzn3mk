...
// تشغيل البوت مباشرة (يستخدم التوكن المخزن في الجلسة)
app.post('/api/start-cloud-bot', (req, res) => {
    if (!req.session.userId) return res.status(401).json({ error: 'Not logged in' });
    if (!req.session.minecraftToken) {
        return res.status(400).json({ error: 'no_minecraft_token', message: '⚠️ لم يتم العثور على حساب ماينكرافت مرتبط بحساب مايكروسوفت الخاص بك. تأكد من أن حسابك يملك Minecraft Java Edition.' });
    }
