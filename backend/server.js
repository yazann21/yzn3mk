...
const { startBot, stopBot, getBotLogs, getBotStats, getBotInventory, sendCommand, deleteBot, botProcesses, getPublicCameraUrl } = require('./bot-starter');
...
const sessions = new Map(); // { sessionId: { userId, username, minecraftToken } }

// Load sessions from file on startup (optional, but keep simple)
// For persistence across restarts, we could save to file, but for now keep in memory.
...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    db.run(`INSERT INTO users (microsoft_id, username, uuid) VALUES (?, ?, ?) ON CONFLICT(microsoft_id) DO UPDATE SET username = excluded.username, uuid = excluded.uuid`,
      [uuid, username, uuid], function(err) {
        if (err) return res.status(500).send('Database error');
        const sessionId = Math.random().toString(36).substring(2, 15);
        sessions.set(sessionId, { userId: uuid, username, minecraftToken });
...
app.get('/api/user/:sessionId', (req, res) => {
  ...
  res.json({ username: s.username, uuid: s.userId });
});

app.get('/api/bots/:sessionId', (req, res) => {
  const s = sessions.get(req.params.sessionId);
  if (!s) return res.status(401).json({ error: 'Not logged in' });
  db.all('SELECT * FROM bots WHERE user_id = ? ORDER BY created_at DESC', [s.userId], (err, bots) => {
    res.json({ bots: bots || [] });
  });
});

app.post('/api/create-bot-cloud', (req, res) => {
  const { sessionId, botName, botType, serverIp, teamNames, version } = req.body;
  const s = sessions.get(sessionId);
  if (!s) return res.status(401).json({ error: 'Not logged in' });
  db.get('SELECT COUNT(*) as count FROM bots WHERE user_id = ? AND is_cloud_bot = 1', [s.userId], (err, row) => {
    ...
    db.run(`INSERT INTO bots (user_id, bot_name, bot_type, server_ip, team_names, version, status, is_cloud_bot) VALUES (?, ?, ?, ?, ?, ?, 'stopped', 1)`,
      [s.userId, botName, botType, serverIp, teamNames || '', version || '1.21.10'], function(err) {
...
app.post('/api/start-cloud-bot', (req, res) => {
  ...
  const s = sessions.get(sessionId);
  if (!s) return res.status(401).json({ error: 'Not logged in' });
  db.get('SELECT * FROM bots WHERE id = ? AND user_id = ?', [botId, s.userId], (err, bot) => {
    if (err || !bot) return res.status(404).json({ error: 'Bot not found' });
    if (botProcesses.has(botId)) return res.json({ success: true });
    startBot(botId, s.username, s.userId, bot.server_ip, bot.bot_type, bot.team_names, bot.version, s.minecraftToken);
...
app.delete('/api/delete-bot', (req, res) => {
  ...
  stopBot(botId);
  deleteBot(botId);
  db.run('DELETE FROM bots WHERE id = ? AND user_id = ?', [botId, s.userId]);
  res.json({ success: true });
});

app.put('/api/update-bot', (req, res) => {
  const { sessionId, botId, botName, botType, serverIp, teamNames, version } = req.body;
  const s = sessions.get(sessionId);
  if (!s) return res.status(401).json({ error: 'Not logged in' });
  stopBot(botId);
  db.run(`UPDATE bots SET bot_name = ?, bot_type = ?, server_ip = ?, team_names = ?, version = ?, status = 'stopped' WHERE id = ? AND user_id = ?`,
    [botName, botType, serverIp, teamNames || '', version || '1.21.10', botId, s.userId]);
  res.json({ success: true });
});
...
app.post('/api/add-task', (req, res) => {
  const { sessionId, botId, command, intervalSeconds } = req.body;
  const s = sessions.get(sessionId);
  if (!s) return res.status(401).json({ error: 'Not logged in' });
...
app.get('/camera/:botId', (req, res) => {
  ...
  const ngrokUrl = getPublicCameraUrl(botId);
  if (ngrokUrl) {
    res.redirect(ngrokUrl);
  } else {
    res.send(`
      <!DOCTYPE html>
      ...
      <body><h2>📷 الكاميرا قيد التشغيل</h2><p>انتظر قليلاً... البوت قد يكون في وضع السكون أو لم يتم تفعيل ngrok.</p><button onclick="location.reload()">تحديث</button></body>
      </html>
    `);
  }
});
