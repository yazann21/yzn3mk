...
// هذه الإعدادات تحل مشكلة 401 على Render
...
// تخزين الجلسات في قاعدة البيانات SQLite (لا تعتمد على المجلد)
app.use(session({
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        db.run(`INSERT INTO users (username, email, password) VALUES (?, ?, ?)`, [username, email, hashed], function(err) {
            if (err) {
                if (err.message.includes('UNIQUE')) return res.status(400).json({ error: 'اسم المستخدم أو البريد موجود' });
...
app.get('/api/bots', (req, res) => {
    if (!req.session.userId) return res.status(401).json({ error: 'Not logged in' });
    db.all('SELECT * FROM bots WHERE user_id = ?', [req.session.userId], (err, bots) => {
...
app.get('/auth/bot-callback', async (req, res) => {
    ...
    let botId = null, flow = null;
    for (let [id, f] of botFlows) { flow = f; botId = id; botFlows.delete(id); break; }
    ...
    res.send('<h2 style="color:white;background:#0a0a1a;text-align:center;padding:50px;">✅ تم التحقق! يمكنك إغلاق النافذة.</h2>');
});
...
app.post('/api/start-cloud-bot', (req, res) => {
    ...
    db.get('SELECT * FROM bots WHERE id = ? AND user_id = ?', [botId, req.session.userId], (err, bot) => {
        if (err || !bot) return res.status(404).json({ error: 'Bot not found' });
        if (!bot.mc_token) return res.status(400).json({ error: 'need_minecraft_auth', message: 'يرجى الضغط على تحقق أولاً' });
...
// باقي المسارات السريعة (stop, delete, update, logs, stats, inventory, command, restart, clear-logs, tasks, camera)
...
app.post('/api/restart-bot', (req, res) => { if (!req.session.userId) return res.status(401).json({ error: 'Not logged in' }); const { botId } = req.body; db.get('SELECT * FROM bots WHERE id = ? AND user_id = ?', [botId, req.session.userId], (err, bot) => { if (!bot) return; stopBot(botId); setTimeout(() => { startBot(botId, bot.bot_name, bot.mc_token, bot.server_ip, bot.bot_type, bot.team_names, bot.version); db.run('UPDATE bots SET status = ? WHERE id = ?', ['online', botId]); }, 1000); res.json({ success: true }); }); });
...
app.get('/camera/:botId', (req, res) => { const botId = parseInt(req.params.botId); res.redirect(`http://localhost:${8080 + botId}`); });
